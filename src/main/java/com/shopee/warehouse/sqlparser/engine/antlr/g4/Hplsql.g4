...
include_stmt :          // INCLUDE statement
     ...
     ;
with_insert_stmt :
       T_WITH with_stmt (T_COMMA with_stmt)* insert_stmt
       ;
with_stmt :
       tbl_name T_AS fullselect_stmt
       ;
insert_stmt :           // INSERT statement
       T_INSERT (T_OVERWRITE T_TABLE | T_INTO T_TABLE?) target_table (select_stmt | insert_stmt_rows)
     ;
target_table :
   target_table_or_partition
;
target_table_or_partition :
       table_name insert_stmt_cols  ? (partition_desc)?
       ;
partition_desc :
       T_PARTITION (T_OPEN_P partition_val (T_COMMA partition_val)* T_CLOSE_P)?
     ;
partition_val :
       ident (T_EQUAL expr_atom)?
       ;
insert_stmt_cols :
       T_OPEN_P column_name (T_COMMA column_name)* T_CLOSE_P
     ;
...
subselect_stmt :
       (T_SELECT | T_SEL) select_list into_clause? from_clause? where_clause? group_by_clause? grouping_sets_clause? (having_clause | qualify_clause)? order_by_clause? select_options? windows?
     ;

windows
    : T_WINDOW namedWindow (',' namedWindow)*
    ;

namedWindow
    : ident T_AS windowSpec
    ;

windowSpec
    : T_OPEN_P expr_func_partition_by_clause? order_by_clause? rows_between_clause? T_CLOSE_P
    ;
...
select_list_alias :
        {!_input.LT(1).getText().equalsIgnoreCase("INTO") && !_input.LT(1).getText().equalsIgnoreCase("FROM")}? T_AS? (select_list_alias_name|T_OPEN_P(select_list_alias_name (T_COMMA select_list_alias_name)*)T_CLOSE_P)
     | T_OPEN_P T_TITLE L_S_STRING T_CLOSE_P
     ;
select_list_alias_name :
       '-'? (L_ID | L_DIGIT_NAME | non_reserved_words) ('.''-'? (L_ID | non_reserved_words))*
     ;
...
from_clause :
       T_FROM from_table_clause (from_join_clause)* (lateral_view)*
     ;
...
from_join_clause :
     ...
     | cross_join_clause from_table_clause (T_ON bool_expr)?
     ;
cross_join_clause :
       T_CROSS T_JOIN
     ;
...
from_alias_clause :
       ...
       T_AS? from_alias_clause_name (T_OPEN_P L_ID (T_COMMA L_ID)* T_CLOSE_P)?
     ;
...
from_alias_clause_name :
  ...
  ;
lateral_view
    : T_LATERAL T_VIEW (T_OUTER)? expr_func from_alias_clause select_list_alias (T_COMMA select_list_alias)*
    ;

table_name :
       (database_name T_DOT)? tbl_name
     ;
database_name :
         '-'? (L_ID  | L_DIGIT_NAME | non_reserved_words)
     ;
tbl_name :
         '-'? (L_ID  | L_DIGIT_NAME | non_reserved_words)
     ;
column_name :
        '-'?(table_name T_DOT)?real_column_name
        ;
real_column_name :
          '-'? (L_ID | L_DIGIT_NAME | non_reserved_words)
        ;
...
group_by_clause :
       T_GROUP T_BY expr (T_COMMA expr)* group_by_with?
     ;
group_by_with :
       T_WITH (T_ROLLUP|T_CUBE)
     ;
grouping_sets_clause :
       T_GROUPING T_SETS T_OPEN_P (grouping_sets_clause_item (T_COMMA grouping_sets_clause_item)*) T_CLOSE_P
     ;
grouping_sets_clause_item :
       expr | T_OPEN_P expr? (T_COMMA grouping_sets_clause_item)* T_CLOSE_P
     ;
...
expr_if :
       T_IF T_OPEN_P bool_expr T_COMMA expr T_COMMA expr T_CLOSE_P
    ;
expr_cursor_attribute :
...
expr_agg_window_func :
     ...
     | T_COLLECT_LIST T_OPEN_P expr T_CLOSE_P expr_func_over_clause?
     | T_COLLECT_SET T_OPEN_P expr T_CLOSE_P expr_func_over_clause?
     ...
     | T_MAX_BY T_OPEN_P expr T_COMMA expr T_CLOSE_P expr_func_over_clause?
     ;
...
expr_func_over_clause :
       T_OVER (ident | T_OPEN_P expr_func_partition_by_clause? order_by_clause? rows_between_clause? T_CLOSE_P)
     ;
rows_between_clause:
       T_ROWS T_BETWEEN rows_between_clause_bound T_AND rows_between_clause_bound
     ;
...
expr_func :
       ident T_OPEN_P expr_func_params? T_CLOSE_P expr_func_child?
     ;
expr_func_child :
       T_OPEN_SB (L_INT | L_S_STRING| L_D_STRING | qident)? T_CLOSE_SB
     ;
...
L_LABEL     : ([a-zA-Z] | L_DIGIT | '_')* ':'
            ;
L_DIGIT_NAME    : L_DIGIT ([a-zA-Z] | L_DIGIT | '_')*
            ;
